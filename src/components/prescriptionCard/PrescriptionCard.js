...
import {getPrescriptions, fetchSinglePrescription, deletePrescription} from '../../state/actions/prescription'
...
const PrescriptionCard =()=> {
  ...
  const handleDelete =(e,id)=>{
    ...
    return (
        <div >
          <UsageFormulaForm handleHide={(e)=>handleHideModal(e,setShow)} show={shows}/>
              <div className="row">
              <UsageFormulaCard handleHide={(e)=>handleHideModal(e,setUsageCard)} usageCard={usageCard} formula={formula}/>
       {prescriptions.map((pres) => (
              <div key={pres._id} className='card'>
                ...
                <div>
                <h2 className="h2">End Date:</h2><p> {pres.end_Date}</p>
                <div className="btn-container">
                <button className="add-formular-btn btn" onClick={(e)=>handleShowModal(e,setShow, pres._id)}
                >Add formula</button>
                <button className="btn view-usage" onClick={(e)=>handleShowModal(e,setUsageCard,pres._id)}>View usage</button>
                <button className="delete-presc btn" onClick={(e)=>{handleDelete(e,pres._id)}}>delete</button>
                </div>
                </div>
                </div>
