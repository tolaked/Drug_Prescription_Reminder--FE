...
export const addFormula = (data,id) => dispatch => {
  ...
  axios({ method: 'POST', url: `http://cors-anywhere.herokuapp.com/https://dashboard.heroku.com/apps/drug-prescription-app/api/v1/formula/add/${id}`,data, headers:{'Access-Control-Allow-Origin': '*',
...
export const getFormula = (id) => dispatch => {
    ...
    const token = Cookie.get('token');
    axios({ method: 'GET', url: `http://cors-anywhere.herokuapp.com/https://dashboard.heroku.com/apps/drug-prescription-app/api/v1/formula/${id}`, headers:{'Access-Control-Allow-Origin': '*',
    'Content-Type': 'application/json',
      'Authorization': token,}})
